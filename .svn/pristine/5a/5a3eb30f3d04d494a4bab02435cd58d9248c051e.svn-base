...
<div class="page-header">
	...
	<div id="header_links" align="right">
		<a style="padding: 3px; ">
		 <input id="searchText"
			type="text" placeholder="输入友情链接标题" style="width: 185px"></input>
		</a>
	
		
		<input type="button" onclick="onFtxxInfoSearch()" value="查询"/>    
		<a class="padding3" id="createUser"
			href="#bpm/admin/newLink" data-role="button"
			data-theme="b" onClick="_execute('target','');" data-icon=""><input
				type="button"
			value="新建">
			</input></a> 
			
			<input type="button" onclick="deleteLinkInfo()" value="删除"/>  			
	</div>
</div>
...
function onFtxxInfoSearch(){
	...
	document.location.href="#bpm/admin/searchLink";
	_execute('target', params);
}


//删除
function deleteLinkInfo(){
    ...
    for(var i=0;i<defpIds.length;i++){
    	var rowData = $("#_LINK_LIST").jqGrid('getRowData',rowid[i]);
    	var j = rowData["id"];
    	id.push(j);
    }
	if(rowid.length == 0){
