...
public class BpmnParse extends Parse {
	...
	public static final String PROPERTYNAME_REFERENCE_RELATION = "referenceRelation";
	...
	public static final String TIME_SETTING_TAG = "timeSetting";
	public static final String TIME_SETTING_MAX_DAYS = "maxDays";
	public static final String TIME_SETTING_WARNING_DAYS = "warningDays";
	public static final String TIME_SETTING_DATE_TYPE = "dateType";
	public static final String TIME_SETTING_URGE_TIMES = "urgeTimes";
	public static final String TIME_SETTING_FREQUENCE_INTERVAL = "frequence";
	public static final String TIME_SETTING_DEAL_IF_TIMEOUT = "dealIfTimeout";
	public static final String TIME_SETTING_NOTIFICATION_TYPE = "notificationType";
	
	public static final String REFERENCE_RELATION_USER = "user";
	public static final String REFERENCE_RELATION_TYPE = "relationship";
	public static final String REFERENCE_RELATION_OPERATION = "operation";
	public static final String CREATOR = "creator";
	public static final String ORGANIZER = "organizer";
	public static final String NULL = "null";
	public static final String  REFERENCE_RELATION_OPERATION_AND = "and";
	public static final String REFERENCE_RELATION_OPERATION_OR = "or";
	
	/*Form Field Automatic*/
	public static final String FORM_FIELD_AUTOMATIC = "formFieldAutomatic";
	public static final String AUTOMATIC_FIELD_INFO = "automaticFieldInfo";
	public static final String AUTOMATIC_FILL_FORM_ID = "fillForm";	
	public static final String AUTOMATIC_FIELD_NAME = "fillFieldName";
	public static final String AUTOMATIC_FIELD_TYPE = "fieldType";
	public static final String AUTOMATIC_FIELD_VALUE = "fieldValue";
	public static final String AUTOMATIC_IS_SKIP = "isSkip";
	public static final String AUTOMATIC_FROM_FORM = "fromForm";
	public static final String AUTOMATIC_FROM_FORM_FILED = "fromFormfield";
	
	protected static final String CANDIDATE_ORDER_SEPERATOR = "-";
	...
	public BpmnParse execute() {
		...
		try {
			...
			// ALL unexpected exceptions should bubble up since they are not
			// handled
			// accordingly by onderlying parse-methods and can't be deployed
			throw new ActivitiException("Error while parsing process: "
					+ e.getMessage(), e);
	...
	public void parseSvgAndJson() {
		Element svg = rootElement.element("svgstring");
		if (svg != null) {
			this.svgString = svg.getText();
		}
		Element json = rootElement.element("jsonstring");
		if (json != null) {
			this.jsonString = json.getText();
		}
	...
	public ProcessDefinitionEntity parseProcess(Element processElement) {
		...
		processDefinition.setProperty(PROPERTYNAME_DOCUMENTATION,
				parseDocumentation(processElement)); // Kept for backwards
														// compatibility. See
														// ACT-1020
		processDefinition
	...
	protected AbstractDataAssociation parseDataInputAssociation(
		...
		if (assignments.isEmpty()) {
			...
			for (Element assigmentElement : dataAssociationElement
					.elements("assignment")) {
				if (assigmentElement.element("from") != null
						&& assigmentElement.element("to") != null) {
	...
	//	processDefinition.setProperty(URGE_MESSAGE,"parseProcessDefinitionStartEvent Messgae line no 1130");
		processDefinition.setProperty(URGE_MESSAGE,parseUrgeMessage(startEventElement,URGE_MESSAGE));
		processDefinition.setProperty(NOTIFICATION_MESSAGE, parseUrgeMessage(startEventElement,NOTIFICATION_MESSAGE));
		Map<String, String> startEventEndScript = parseEndScript(startEventElement);
		
		if (startEventEndScript != null && startEventEndScript.size() > 0) {
			processDefinition.setEndScriptExpression(expressionManager
					.createExpression(startEventEndScript.get("script")));
			processDefinition.setEndScriptNameExpression(expressionManager
					.createExpression(startEventEndScript.get("functionName")));
		}
		...
		if (timerEventDefinition != null) {
			...
			addEventSubscriptionDeclaration(messageDefinition,
					processDefinition, startEventElement);
		} 
	}
	...
	protected void parseScopeStartEvent(ActivityImpl startEventActivity,
		...
		Element messageEventDefinition = startEventElement
				.element("messageEventDefinition");
		Element signalEventDefinition = startEventElement
		...
		if (isTriggeredByEvent) { // event subprocess
			...
			String isInterrupting = startEventElement
					.attribute("isInterrupting");
			if (isInterrupting != null
					&& (isInterrupting.equals("false") || isInterrupting
							.equals("FALSE"))) {
			...
			if (errorEventDefinition != null) {
				if (!activityBehavior.isInterrupting()) {
					addError(
							"error start event of event subprocess must be interrupting",
							startEventElement);
				}
				if (scope.getProperty(PROPERTYNAME_INITIAL) == null) {
					scope.setProperty(PROPERTYNAME_INITIAL, startEventActivity);
					parseErrorStartEventDefinition(errorEventDefinition,
							startEventActivity, catchingScope);
				} else {
					addError(
				...
				eventSubscriptionDeclaration.setActivityId(startEventActivity
						.getId());
				eventSubscriptionDeclaration.setStartEvent(false);
				addEventSubscriptionDeclaration(eventSubscriptionDeclaration,
						catchingScope, messageEventDefinition);
				...
				eventSubscriptionDeclaration.setStartEvent(false);
				addEventSubscriptionDeclaration(eventSubscriptionDeclaration,
						catchingScope, signalEventDefinition);
	...
	protected EventSubscriptionDeclaration parseMessageEventDefinition(
		...
		if (messageRef == null) {
			addError("attribute 'messageRef' is required",
					messageEventDefinition);
		}
		...
		return new EventSubscriptionDeclaration(messageDefinition.getName(),
				"message");
	}

	@SuppressWarnings("unchecked")
	protected void addEventSubscriptionDeclaration(
			EventSubscriptionDeclaration subscription, ScopeImpl scope,
			Element element) {
		...
		if (eventDefinitions == null) {
			...
			scope.setProperty(PROPERTYNAME_EVENT_SUBSCRIPTION_DECLARATION,
					eventDefinitions);
		} else {
			...
			if (subscription.getEventType().equals("message")) {
				for (EventSubscriptionDeclaration eventDefinition : eventDefinitions) {
					if (eventDefinition.getEventType().equals("message")
							&& eventDefinition.getEventName().equals(
									subscription.getEventName())
							&& eventDefinition.isStartEvent() == subscription
									.isStartEvent()) {
						addError(
	...
	protected void parseActivity(Element activityElement,
		...
		if (activityElement.getTagName().equals("exclusiveGateway")) {
			...
			activity = parseUserTask(activityElement, parentElement,
					scopeElement);
	...
	public void validateExclusiveGateway(ActivityImpl activity) {
		if (activity.getOutgoingTransitions().size() == 0) {
			// TODO: double check if this is valid (I think in Activiti yes,
			// since we need start events we will need an end event as well)
			addError("Exclusive Gateway '" + activity.getId()
					+ "' has no outgoing sequence flows.", null);
		} else if (activity.getOutgoingTransitions().size() == 1) {
			PvmTransition flow = activity.getOutgoingTransitions().get(0);
			Condition condition = (Condition) flow
			...
			String defaultSequenceFlow = (String) activity
					.getProperty("default");
			boolean hasDefaultFlow = defaultSequenceFlow != null
					&& defaultSequenceFlow.length() > 0;
			...
			for (PvmTransition flow : activity.getOutgoingTransitions()) {
				Condition condition = (Condition) flow
						.getProperty(BpmnParse.PROPERTYNAME_CONDITION);
				boolean isDefaultFlow = flow.getId() != null
			...
			if (hasDefaultFlow || flowsWithoutCondition.size() > 1) {
				// if we either have a default flow (then no flows without
				// conditions are valid at all) or if we have more than one flow
				// without condition this is an error
				...
				// Havinf no default and exactly one flow without condition this
				// is considered the default one now (to not break backward
				// compatibility)
	...
	public ActivityImpl parseIntermediateCatchEvent(
		...
		if (timerEventDefinition != null) {
			...
			parseIntemediateMessageEventDefinition(messageEventDefinition,
					nestedActivity, isAfterEventBasedGateway);
		} else {
	...
	protected void parseIntemediateMessageEventDefinition(
		...
		if (isAfterEventBasedGateway) {
			...
			addEventSubscriptionDeclaration(messageDefinition,
					nestedActivity.getParent(), messageEventDefinition);
		} else {
		...
		for (BpmnParseListener parseListener : parseListeners) {
			parseListener.parseIntermediateMessageCatchEventDefinition(
					messageEventDefinition, nestedActivity);
		}
	}
	...
	public ActivityImpl createActivityOnScope(Element activityElement,
		...
		activity.setProperty("isArchive", activityElement.attribute("isarchive"));
		activity.setProperty("readerOption", activityElement.attribute("readeroption"));
		activity.setProperty("dynamicReader", activityElement.attribute("dynamicReader"));
		activity.setProperty("dynamicReaderType", activityElement.attribute("dynamicReaderType"));
		activity.setProperty("formName", activityElement.attribute("formName"));

		//set start event properties
		if (activityElement.getTagName() == "startEvent" ) {
			Map<String, String> endScriptMap = parseEndScript(activityElement);
			activity.setProperty("endScript", endScriptMap);
			activity.setProperty("dynamicOrganizer", activityElement.attribute("dynamicOrganizer"));
			activity.setProperty("dynamicOrganizerType", activityElement.attribute("dynamicOrganizerType"));
			activity.setProperty(URGE_MESSAGE, parseUrgeMessage(activityElement,URGE_MESSAGE));
			activity.setProperty(NOTIFICATION_MESSAGE, parseUrgeMessage(activityElement,NOTIFICATION_MESSAGE));
		}
		//set start scripts present in end node
		if(activityElement.getTagName() == "endEvent") {
			Map<String, String> startScriptMap = parseStartScript(activityElement);
			activity.setProperty("endEventStartScript", startScriptMap);
		}
	...
	public String parseUrgeMessage(Element element,String tagName) {
		List<Element> docElements = element.elements(tagName);
		if (docElements.isEmpty()) {
			return null;
		}
	...
	protected static final String START_SCRIPT = "startScript";
	protected static final String END_SCRIPT = "endScript";
	...
	public static final String DYNAMIC_NEXT_ORGANIZER = "dynamicOrganizer";
	protected static final String DYNAMIC_NEXT_READER = "dynamicReader";
	public static final String DYNAMIC_ORGNIZER_TYPE = "dynamicOrganizerType";
	protected static final String DYNAMIC_READER_TYPE = "dynamicReaderType";
	protected static final String FORM_NAME = "formName";
	protected static final String URGE_MESSAGE = "urgeMessage";
	protected static final String NOTIFICATION_MESSAGE = "notificationMessage";
	/**
	...
	public ActivityImpl parseUserTask(Element userTaskElement,
		...
		activity.setProperty("processDefinition", parentElement.attribute("processName"));

		TaskDefinition taskDefinition = parseTaskDefinition(userTaskElement,parentElement, activity.getId(),(ProcessDefinitionEntity) scope.getProcessDefinition());
	...
	public TaskDefinition parseTaskDefinition(Element taskElement,
			Element parentElement, String taskDefinitionKey,
			ProcessDefinitionEntity processDefinition) {
		...
		OperatingFunctionHandler operatingFunctionHandler = new OperatingFunctionHandler(
				taskElement);
		
		// operatingFunctionHandler.parseConfiguration(taskElement);

		TaskDefinition taskDefinition = new TaskDefinition(taskFormHandler);
		taskDefinition.setOperatingFunctions(operatingFunctionHandler
				.getOperatingFunctions());
		...
		taskDefinition.setTimeSettingDetails(setTimeSettingDetails(taskElement));
		taskDefinition.setFormFieldAutomatic(setFormFieldAutomatic(taskElement));
		String formName = taskElement.attribute(FORM_NAME);
		String isOpinion = taskElement.attribute("opinion");
		taskDefinition.setIsOpinion(isOpinion);
		if (formName != null) {
			taskDefinition.setFormName(expressionManager
					.createExpression(formName));
		}
		String skipRepeat = taskElement.attribute("repeatedoperation");
		if (skipRepeat != null) {
			taskDefinition.setSkipRepeatedExpression(expressionManager
					.createExpression(skipRepeat));
		}
		
		String nodeType = taskElement.attribute("nodeType");
		if (nodeType != null) {
			taskDefinition.setNodeTypeExpression(expressionManager
					.createExpression(nodeType));
		}

		String dynamicOrganizer = taskElement.attribute(DYNAMIC_NEXT_ORGANIZER);
		if (dynamicOrganizer != null) {
			taskDefinition.setDynamicOrganizer(expressionManager
					.createExpression(dynamicOrganizer));
			String dynamicOrganizerType = taskElement.attribute(DYNAMIC_ORGNIZER_TYPE);
			//Set the type of dynamic organizer(type may role,user,group,etc)
			if(dynamicOrganizerType !=null){
			taskDefinition.setDynamicOrganizerType(expressionManager
					.createExpression(dynamicOrganizerType));
			}
		}

		String dynamicReader = taskElement.attribute(DYNAMIC_NEXT_READER);
		if (dynamicReader != null) {
			taskDefinition.setDynamicReader(expressionManager
					.createExpression(dynamicReader));
			String dynamicReaderType = taskElement.attribute(DYNAMIC_READER_TYPE);
			//Set the type of dynamic reader(type may role,user,group,etc)
			if(dynamicReaderType!= null){
			taskDefinition.setDynamicReaderType(expressionManager
					.createExpression(dynamicReaderType));
			}
		}

		String skipEmpty = taskElement.attribute("emptypersonnel");
		if (skipEmpty != null) {

			taskDefinition.setSkipEmptyExpression(expressionManager
					.createExpression(skipEmpty));
		}
		...
		String signOffType = getSignOffType(taskElement);
		taskDefinition.setSignOffTypeExpression(expressionManager
				.createExpression(signOffType));
		String isForStartNode = taskElement.attribute("isForStartNode");
		if(isForStartNode != null){
			taskDefinition.setIsForStartNode(expressionManager
					.createExpression(isForStartNode));
		}

		taskDefinition = setAssigneeIfApplicable(taskElement,
				getOrganizerElements(taskElement), taskDefinition,
				getMinOrderFromElements(taskElement.elements(POTENTIAL_OWNER)));
		Map<String, String> startScriptMap = parseStartScript(taskElement);

		if (startScriptMap != null && startScriptMap.size() > 0) {
			taskDefinition.setStartScriptExpression(expressionManager
					.createExpression(startScriptMap.get("script")));
			taskDefinition.setStartScriptNameExpression(expressionManager
					.createExpression(startScriptMap.get("functionName")));
		}

		List<Element> startEventElements = parentElement.elements("startEvent");
		for (Element startEventElement : startEventElements) {
			parseProcessAdminReader(startEventElement, taskDefinition);
			processDefinition.setProperty(URGE_MESSAGE, parseUrgeMessage(startEventElement,URGE_MESSAGE));
			processDefinition.setProperty(NOTIFICATION_MESSAGE, parseUrgeMessage(startEventElement,NOTIFICATION_MESSAGE));
			//processDefinition.setProperty(URGE_MESSAGE, "parseTaskDefinition line no 2801");
		}
		...
		Map<String, String> endScriptMap = parseEndScript(taskElement);

		if (endScriptMap != null && endScriptMap.size() > 0) {
			taskDefinition.setEndScriptExpression(expressionManager
					.createExpression(endScriptMap.get("script")));
			taskDefinition.setEndScriptNameExpression(expressionManager
					.createExpression(endScriptMap.get("functionName")));
		}

		return taskDefinition;
	}

	private List<Element> getOrganizerElements(Element taskElement) {
		List<Element> organizers = new ArrayList<Element>();
		List<Element> humanPerformerElements = taskElement
				.elements(HUMAN_PERFORMER);
		String organizerRoleName = TaskRole.ORGANIZER.getName();
		for (Element humanPerformerElement : humanPerformerElements) {
			if (humanPerformerElement.attribute("role", organizerRoleName)
					.equalsIgnoreCase(organizerRoleName)) {
				organizers.add(humanPerformerElement);
			}
	...
	private Map<String, String> parseStartScript(Element taskElement) {
		List<Element> startScripts = taskElement.elements(START_SCRIPT);
		Map<String, String> startScript = new HashMap<String, String>();

		if (startScripts != null && startScripts.size() > 0) {
			Element startScriptElement = startScripts.get(0);

			List<Element> scriptFunctionName = startScriptElement
					.elements("functionName");
			if (scriptFunctionName != null && scriptFunctionName.size() > 0) {
				String functionName = scriptFunctionName.get(0).getText();
				startScript.put("functionName", functionName);
			}

			List<Element> scriptContent = startScriptElement.elements("script");
			if (scriptFunctionName != null && scriptFunctionName.size() > 0) {
				String script = scriptContent.get(0).getText();
				startScript.put("script", script);

			}

			// endScript=endScriptElement.getText();
		}
		return startScript;
	}

	private Map<String, String> parseEndScript(Element taskElement) {
		List<Element> endScripts = taskElement.elements(END_SCRIPT);
		Map<String, String> endScript = new HashMap<String, String>();

		if (endScripts != null && endScripts.size() > 0) {
			Element startScriptElement = endScripts.get(0);

			List<Element> scriptFunctionName = startScriptElement
					.elements("functionName");
			if (scriptFunctionName != null && scriptFunctionName.size() > 0) {
				String functionName = scriptFunctionName.get(0).getText();
				endScript.put("functionName", functionName);
			}
			List<Element> scriptContent = startScriptElement.elements("script");
			if (scriptFunctionName != null && scriptFunctionName.size() > 0) {
				String script = scriptContent.get(0).getText();
				endScript.put("script", script);
	...
	private String getSignOffType(Element taskElement) {
		// If sign off type not indicated or empty, default will be
		// TransactorType.SINGLE_PLAYER_SINGLE_SIGNOFF
		String signOffType = taskElement.attribute("nodeType",
				TransactorType.SINGLE_PLAYER_SINGLE_SIGNOFF.getStateCodeStr());
		if (StringUtil.isEmptyString(signOffType)) {
			LOGGER.log(Level.INFO,
					"sign off type set as empty while creating so setting default-----");
			signOffType = TransactorType.SINGLE_PLAYER_SINGLE_SIGNOFF
					.getStateCodeStr();
		}
		return signOffType;
	}
	/*protected void parseAdminForProcess(Element taskElement,
			TaskDefinition taskDefinition) {
		List<Element> humanPerformerElements = taskElement
				.elements(HUMAN_PERFORMER);

		for (Element humanPerformerElement : humanPerformerElements) {
			String taskRole = humanPerformerElement.attribute("role");
			if (!StringUtil.isEmptyString(taskRole)
					&& taskRole
							.equalsIgnoreCase(TaskRole.WORKFLOW_ADMINISTRATOR
									.getName())) {

				Element resAssExpr = humanPerformerElement
						.element(RESOURCE_ASSIGNMENT_EXPR);
				if (resAssExpr != null) {
					Element feElement = resAssExpr.element(FORMAL_EXPRESSION);

					if (feElement != null) {
						Expression userId = expressionManager
								.createExpression(feElement.getText());
						taskDefinition.addAdminUserIdExpression(userId);
					}
				}
			}
		}
	...
	/*protected void parseReaderForProcess(Element taskElement,
			TaskDefinition taskDefinition) {
		...
		for (Element humanPerformerElement : humanPerformerElements) {
			String taskRole = humanPerformerElement.attribute("role");
			if (!StringUtil.isEmptyString(taskRole)
					&& taskRole.equalsIgnoreCase(TaskRole.READER.getName())) {
				Element resAssExpr = humanPerformerElement
						.element(RESOURCE_ASSIGNMENT_EXPR);
				if (resAssExpr != null) {
					Element feElement = resAssExpr.element(FORMAL_EXPRESSION);
					if (feElement != null) {
						...
						taskDefinition.addReaderUserIdExpression(userId);
					}
				}
		...
		for (Element performerElement : potentialOwnerElements) {
			String groupType = performerElement.attribute("type", "0");

			if (groupType.equals("0"))
				groupType = "group";

			Element raeElement = performerElement
					.element(RESOURCE_ASSIGNMENT_EXPR);
			if (raeElement != null) {
				Element feElement = raeElement.element(FORMAL_EXPRESSION);
				String order = feElement.attribute("evaluatesToTypeRef", "1");
				// Added superior subordinate functionality
				String superior = feElement.attribute("superior", "false");
				String subordiante = feElement
						.attribute("subordinate", "false");
				if (feElement != null) {
					List<String> assignmentExpressions = parseCommaSeparatedList(feElement
							.getText());
					for (String assignmentExpression : assignmentExpressions) {
						assignmentExpression = assignmentExpression.trim();
						String groupAssignementId = formIdentityOrderExpression(
								assignmentExpression, order, superior,
								subordiante);

						if (groupType != null && !groupType.isEmpty())
							groupAssignementId = groupAssignementId.concat(
									TaskDefinition.CANDIDATE_ORDER_SEPERATOR)
									.concat(groupType);

						Expression groupId = expressionManager
								.createExpression(groupAssignementId);
						taskDefinition.addReaderGroupIdExpression(groupId);
					}
				}
			}
		}
	...
	protected void parseProcessAdminReader(Element taskElement,TaskDefinition taskDefinition) {
		parseAdminAndReaderForProcess(taskElement, taskDefinition , TaskRole.WORKFLOW_ADMINISTRATOR.getName());
		parseAdminAndReaderForProcess(taskElement, taskDefinition , TaskRole.READER.getName());
//		parseAdminForProcess(taskElement, taskDefinition);
//		parseReaderForProcess(taskElement, taskDefinition);
	}

	/**
	 * parsing workflow admin and reader in process in start event
	 * @param taskElement
	 * @param taskDefinition
	 * @param adminAndReader
	 */
	private void parseAdminAndReaderForProcess(Element taskElement,TaskDefinition taskDefinition, String adminAndReader) {
		// TODO Auto-generated method stub
		List<Element> humanPerformerElements = taskElement.elements(HUMAN_PERFORMER);
		for (Element humanPerformerElement : humanPerformerElements) {
			String taskRole = humanPerformerElement.attribute("role");
			if (!StringUtil.isEmptyString(taskRole)	&& taskRole.equalsIgnoreCase(adminAndReader)) {
				Element resAssExpr = humanPerformerElement.element(RESOURCE_ASSIGNMENT_EXPR);
				if (resAssExpr != null) {
					Element feElement = resAssExpr.element(FORMAL_EXPRESSION);
					if (feElement != null) {
						Expression userId = expressionManager.createExpression(feElement.getText());
						taskDefinition.addAdminUserIdExpression(userId);
						if(adminAndReader.equalsIgnoreCase(TaskRole.READER.getName())) {
							taskDefinition.addReaderUserIdExpression(userId);
						} else {
							taskDefinition.addAdminUserIdExpression(userId);;
						}						
		...
		List<Element> potentialOwnerElements = taskElement
				.elements(POTENTIAL_OWNER);
		for (Element performerElement : potentialOwnerElements) {
			String taskRole = performerElement.attribute("role");
			if (!StringUtil.isEmptyString(taskRole)	&& taskRole.equalsIgnoreCase(adminAndReader)) {
				String groupType = performerElement.attribute("type", "0");
				if (groupType.equals("0"))
					groupType = "group";
					...
					if (feElement != null) {
						...
						for (String assignmentExpression : assignmentExpressions) {
							...
							if(adminAndReader.equalsIgnoreCase(TaskRole.READER.getName())) {
								...
								taskDefinition.addAdminGroupIdExpression(groupId);
							}
						}
					}
				}
	...
	protected void parseHumanPerformer(Element taskElement,
		...
		// We are overriding the default activiti behaviour which allows only
		// one assignee per task
		// if (humanPerformerElements.size() > 1) {
		// addError("Invalid task definition: multiple " + HUMAN_PERFORMER +
	...
	private int getMinOrderFromElements(List<Element> performerElements) {
		int minOrder = 0;
		int i = 0;
		if (performerElements != null) {
			// Expression assigneeExpression = null;
			for (Element humanPerformerElement : performerElements) {
				Element raeElement = humanPerformerElement
						.element(RESOURCE_ASSIGNMENT_EXPR);
				if (raeElement != null) {
					Element feElement = raeElement.element(FORMAL_EXPRESSION);
					int order = Integer.parseInt(feElement.attribute(
							"evaluatesToTypeRef", "1"));
					if (i == 0) {
						minOrder = order;
					}
					if (order <= minOrder) {
						// assigneeExpression =
						// expressionManager.createExpression(feElement.getText());
						minOrder = order;
					}
				}
				i++;
		...
		LOGGER.log(Level.INFO,
				"The min order from prev potential owners list-----" + minOrder);
		return minOrder;
	}

	/**
	 * <p>
	 * Check if assignee is applicable while parsing itself based on the sign
	 * off type
	 * </p>
	 * 
	 * @param taskElement
	 * @param humanPerformerElements
	 * @param taskDefinition
	 * @return
	 * 
	 * @author madan
	 */
	protected TaskDefinition setAssigneeIfApplicable(Element taskElement,
			List<Element> humanPerformerOrganizerElements,
			TaskDefinition taskDefinition, int MinOrderForGroups) {
		int minOrder = MinOrderForGroups;
		int i = 0;
		Expression assigneeExpression = null;
		LOGGER.log(Level.INFO, "Check if assignee can be set while parsing----"
				+ taskDefinition.getKey());
		String signOffType = getSignOffType(taskElement);
		LOGGER.log(Level.INFO, "sign off type-----" + signOffType);
		// These two sign off types have only single assignee or single assignee
		// at a time, so we can set him as assignee
		if (signOffType != null
				&& (signOffType.equals(TransactorType.MULTI_SEQUENCE_SIGNOFF
						.getStateCodeStr()))) {
			LOGGER.log(Level.INFO,
					"Assignee can be set for the sign off type---");
			// For both types, if we get min order, it will be ok, since even
			// for single sign off it will have a dummy order
			for (Element humanPerformerElement : humanPerformerOrganizerElements) {
				Element raeElement = humanPerformerElement
						.element(RESOURCE_ASSIGNMENT_EXPR);
				if (raeElement != null) {
					Element feElement = raeElement.element(FORMAL_EXPRESSION);
					...
					if (order <= minOrder) {
						assigneeExpression = expressionManager
								.createExpression(feElement.getText());
						minOrder = order;
					}
				}
	...
	protected void parseHumanPerformerResourceAssignment(
		...
		Element raeElement = performerElement.element(RESOURCE_ASSIGNMENT_EXPR);
		// String order = performerElement.attribute("order", "0");
		if (raeElement != null) {
			Element feElement = raeElement.element(FORMAL_EXPRESSION);
			String order = feElement.attribute("evaluatesToTypeRef", "1");
			// Added superior subordinate functionality
			String superior = feElement.attribute("superior", "false");
			String subordiante = feElement.attribute("subordinate", "false");
			if (feElement != null) {
				// taskDefinition.setAssigneeExpression(expressionManager.createExpression(feElement.getText()));
				// This is the actual step where we cut off assigning assignee
				// directly. We add entry to identity links
				// and assignee will be assigned based on 4 types of node sign
				// off
				List<String> assignmentExpressions = parseCommaSeparatedList(feElement
						.getText());
				for (String assignmentExpression : assignmentExpressions) {
					assignmentExpression = assignmentExpression.trim();
					// String userAssignementId =
					// getAssignmentId(assignmentExpression, USER_PREFIX);
					// We are not having any prefix since we have human
					// performer directly for user. So above step not needed.
					String userAssignementId = formIdentityOrderExpression(
							assignmentExpression, order, superior, subordiante);
					LOGGER.log(Level.INFO,
							"Setting user identity expression----"
									+ userAssignementId);
					taskDefinition = addUserIdentityExpression(taskDefinition,
							performerElement, userAssignementId);
	...
	protected void parsePotentialOwnerResourceAssignment(
		...
		if (raeElement != null) {
			...
			String order = feElement.attribute("evaluatesToTypeRef", "1");
			// Added superior subordinate functionality
			String superior = feElement.attribute("superior", "true");
			String subordiante = feElement.attribute("subordinate", "true");
			if (feElement != null) {
				List<String> assignmentExpressions = parseCommaSeparatedList(feElement
						.getText());
				for (String assignmentExpression : assignmentExpressions) {
					...
					if (assignmentExpression.startsWith(USER_PREFIX)) {
						String userAssignementId = formIdentityOrderExpression(
								getAssignmentId(assignmentExpression,
										USER_PREFIX), order, superior,
								subordiante);
						taskDefinition = addUserIdentityExpression(
								taskDefinition, performerElement,
								userAssignementId);
						// taskDefinition.addCandidateUserIdExpression(expressionManager.createExpression(userAssignementId));
					} else if (assignmentExpression.startsWith(GROUP_PREFIX)) {
						String groupAssignementId = formIdentityOrderExpression(
								getAssignmentId(assignmentExpression,
										GROUP_PREFIX), order, superior,
								subordiante);
						if (groupType != null && !groupType.isEmpty())
							groupAssignementId = groupAssignementId.concat(
									TaskDefinition.CANDIDATE_ORDER_SEPERATOR)
									.concat(groupType);
						taskDefinition = addGroupIdentityExpression(
								taskDefinition, performerElement,
								groupAssignementId);
						// taskDefinition.addCandidateGroupIdExpression(expressionManager.createExpression(groupAssignementId));
					} else { // default: given string is a goupId, as-is.
						// taskDefinition.addCandidateGroupIdExpression(expressionManager.createExpression(formIdentityOrderExpression(assignmentExpression,
						// order)));
						String groupAssignementId = formIdentityOrderExpression(
								assignmentExpression, order, superior,
								subordiante);
	...
	/**
	 * <p>
	 * We combine candidate user and his order using a separator character and
	 * assign, we will split while assigning actual assignee and order value for
	 * task identity link
	 * </p>
	 * 
	 * @param expression
	 *            assignee expression
	 * @param order
	 *            the order for the assignee
	 * @return the combined expression
	 * 
	 * @author madan
	 */
	protected String formIdentityOrderExpression(String expression,
			String order, String superior, String subordiante) {
		String expWithOrder = expression.concat(
				TaskDefinition.CANDIDATE_ORDER_SEPERATOR).concat(order);
		String expWithSuperior = expWithOrder.concat(
				TaskDefinition.CANDIDATE_ORDER_SEPERATOR).concat(superior);
		String expWithSupordinate = expWithSuperior.concat(
				TaskDefinition.CANDIDATE_ORDER_SEPERATOR).concat(subordiante);
		return expWithSupordinate;
	}

	private TaskDefinition addUserIdentityExpression(
			TaskDefinition taskDefinition, Element resourceElement, String value) {
		Expression expression = expressionManager.createExpression(value);
		String role = getTaskRole(resourceElement);
		if (role.equalsIgnoreCase(IdentityLinkType.ORGANIZER)) {
			taskDefinition.addCandidateUserIdExpression(expression);
		} else if (role.equalsIgnoreCase(IdentityLinkType.READER)) {
			taskDefinition.addReaderUserIdExpression(expression);
		}
		return taskDefinition;
	}

	private TaskDefinition addGroupIdentityExpression(
			TaskDefinition taskDefinition, Element resourceElement, String value) {
		Expression expression = expressionManager.createExpression(value);
		String role = getTaskRole(resourceElement);
		if (role.equalsIgnoreCase(IdentityLinkType.ORGANIZER)) {
			taskDefinition.addCandidateGroupIdExpression(expression);
		} else if (role.equalsIgnoreCase(IdentityLinkType.READER)) {
			taskDefinition.addReaderGroupIdExpression(expression);
		}
		return taskDefinition;
	}

	private String getTaskRole(Element resourceElement) {
		return resourceElement.attribute("role", IdentityLinkType.ORGANIZER);
	}
	...
	public void parseEndEvents(Element parentElement, ScopeImpl scope) {
		for (Element endEventElement : parentElement.elements("endEvent")) {
			...
			Element terminateEventDefinition = endEventElement
			...
			if (errorEventDefinition != null) { // error end event
				...
				if (errorRef == null || "".equals(errorRef)) {
					...
					if (error != null
							&& (error.getErrorCode() == null || "".equals(error
									.getErrorCode()))) {
						addError(
				...
				TaskDefinition taskDefinition = parseEndEventDefinition(
						endEventElement, parentElement, activity.getId(),
	...
	public TaskDefinition parseEndEventDefinition(Element taskElement,
			Element parentElement, String taskDefinitionKey,
			ProcessDefinitionEntity processDefinition) {
		...
		List<Element> endEventElements = parentElement.elements("endEvent");
		for (Element endEventElement : endEventElements) {
			parseAdminAndReaderForProcess(endEventElement, taskDefinition,"reader");
		}
	...
	public void parseBoundaryEvents(Element parentElement,
		...
		for (Element boundaryEventElement : parentElement
			...
			Element compensateEventDefinition = boundaryEventElement
					.element("compensateEventDefinition");
			Element messageEventDefinition = boundaryEventElement
					.element("messageEventDefinition");
			if (timerEventDefinition != null) {
				behavior = new BoundaryEventActivityBehavior(interrupting,
						nestedActivity.getId());
				...
				behavior = new BoundaryEventActivityBehavior(interrupting,
						nestedActivity.getId());
				parseBoundaryMessageEventDefinition(messageEventDefinition,
						interrupting, nestedActivity);
			} else {
	...
	public void parseBoundarySignalEventDefinition(Element element,
		...
		addEventSubscriptionDeclaration(signalDefinition,
				signalActivity.getParent(), element);
		...
		for (BpmnParseListener parseListener : parseListeners) {
			parseListener.parseBoundarySignalEventDefinition(element,
	...
	public void parseBoundaryMessageEventDefinition(Element element,
		...
		addEventSubscriptionDeclaration(messageEventDefinition,
				messageActivity.getParent(), element);
		...
		for (BpmnParseListener parseListener : parseListeners) {
			parseListener.parseBoundaryMessageEventDefinition(element,
					interrupting, messageActivity);
		}
	...
	protected void parseIntemediateSignalEventDefinition(Element element,
			ActivityImpl signalActivity, boolean isAfterEventBasedGateway) {
		...
		for (BpmnParseListener parseListener : parseListeners) {
			parseListener.parseIntermediateSignalCatchEventDefinition(element,
					signalActivity);
		}
	}
	...
	protected EventSubscriptionDeclaration parseSignalEventDefinition(
		...
		if (signalRef == null) {
			...
			SignalDefinition signalDefinition = signals
					.get(resolveName(signalRef));
			if (signalDefinition == null) {
				addError("Could not find signal with id '" + signalRef + "'",
						signalEventDefinitionElement);
			}
			EventSubscriptionDeclaration signalEventDefinition = new EventSubscriptionDeclaration(
					signalDefinition.getName(), "signal");
	...
	protected void parseIntemediateTimerEventDefinition(
	...
	public ActivityImpl parseSubProcess(Element subProcessElement,
		...
		activity.setProperty("processDefinition", subProcessElement.attribute("processName"));
	...
	public void parseSequenceFlow(Element processElement, ScopeImpl scope)  {
		for (Element sequenceFlowElement : processElement
			...
			/*List<Map<String,Object>> relationInfo = new ArrayList<Map<String,Object>>();
			if(!transactorsrelationship.isEmpty()){
				 relationInfo = CommonUtil.convertJsonToList(transactorsrelationship);
			}*/
			...
			if (sourceActivity == null) {
				...
				addError("Invalid destination '" + destinationRef
						...
						sequenceFlowElement);
				/*
				 * } else if(sourceActivity.getActivityBehavior() instanceof
				 * EventBasedGatewayActivityBehavior) { // ignore
				 */
				...
				transition.setProperty("relationInfo", sequenceFlowElement.attribute("transactorsrelationship"));
	...
	public void parseSequenceFlowConditionExpression(Element seqFlowElement,
		...
		if (conditionExprElement != null) {
			...
			// String expression =
			// "{'type':'javascript','value':[{'val':'var&#37;20type&#37;20&#37;3D&#37;20&#37;24&#37;7Bstring&#37;7D&#37;2B&#37;22manbage&#37;22&#37;3B'}]}";
			String type = conditionExprElement.attributeNS(BpmnParser.XSI_NS,
					"type");
			if (type != null) {
				String value = type.contains(":") ? resolveName(type)
						: BpmnParser.BPMN20_NS + ":" + type;
			...
			org.json.JSONObject expressionData = null;
			String expressionType = "";
			try {
				 
				expressionData = new org.json.JSONObject(expression);
				expressionType = expressionData.getString("type");
				org.json.JSONObject expValue = (org.json.JSONObject) expressionData
						.getJSONArray("value").get(0);
				expression = java.net.URLDecoder.decode(
						expValue.getString("val"), "UTF-8");
				
				if (expressionType.equalsIgnoreCase("javascript")) {
					Condition scriptExpressionCondition = new ScriptCondition(
							expression, "javascript");
					seqFlow.setProperty(PROPERTYNAME_CONDITION_TEXT, expression);
					seqFlow.setProperty(PROPERTYNAME_CONDITION,
							scriptExpressionCondition);
				} else {
	...
	public void parseBPMNEdge(Element bpmnEdgeElement) {
		...
		if (sequenceFlowId != null && !"".equals(sequenceFlowId)) {
			if (sequenceFlows != null
					&& sequenceFlows.containsKey(sequenceFlowId)) {
	...
	public Boolean parseBooleanAttribute(String booleanText) {
		if ("true".equals(booleanText) || "enabled".equals(booleanText)
				...
				|| "yes".equals(booleanText) || "1".equals(booleanText)) {
			return Boolean.TRUE;
		}
		if ("false".equals(booleanText) || "disabled".equals(booleanText)
				|| "off".equals(booleanText) || "inactive".equals(booleanText)
				|| "no".equals(booleanText) || "0".equals(booleanText)
				|| "null".equals(booleanText) || booleanText == null) {
			return Boolean.FALSE;
		}
		return null;
	}

	public Boolean checkAndParseBooleanAttribute(String booleanText) {
		Boolean attr = parseBooleanAttribute(booleanText);
		if (attr == null) {
			throw new BpmException(
					"Invalid boolean value, "
							+ booleanText
							+ " Must be one of {on|yes|true|enabled|active|off|no|false|disabled|inactive|1|0}");
		}
		return attr;
	}
	...
	private List<Map<String,String>> setFormFieldAutomatic(Element element){
		List<Map<String,String>> formFieldAutomaticList = new ArrayList<Map<String,String>>();		
		List<Element> auoMaticFormFields = element.elements(FORM_FIELD_AUTOMATIC);
		for(Element autoMaticFormEl : auoMaticFormFields){
			
			List<Element> auoMaticFieldInfo = autoMaticFormEl.elements(AUTOMATIC_FIELD_INFO);
			for(Element auoMaticFieldEl : auoMaticFieldInfo){
				Map<String,String> formFieldAutomatic = new HashMap<String,String>();
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FILL_FORM_ID,formFieldAutomatic);
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FIELD_NAME,formFieldAutomatic);
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FIELD_TYPE,formFieldAutomatic);
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FIELD_VALUE,formFieldAutomatic);
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FROM_FORM,formFieldAutomatic);
				getElemtntFunctionValue(auoMaticFieldEl,AUTOMATIC_FROM_FORM_FILED,formFieldAutomatic);
				formFieldAutomaticList.add(formFieldAutomatic);
			}
			
			
		}
		return formFieldAutomaticList;
		
	}
	
	private Map<String,String> setTimeSettingDetails(Element element){
		Map<String,String> timeSettingVal = new HashMap<String,String>();
		List<Element> timeSettingFunctionEls = element.elements(TIME_SETTING_TAG);
		for (Element timeSettingEl : timeSettingFunctionEls) {
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_MAX_DAYS,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_WARNING_DAYS,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_DATE_TYPE,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_URGE_TIMES,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_FREQUENCE_INTERVAL,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_DEAL_IF_TIMEOUT,timeSettingVal);
			getElemtntFunctionValue(timeSettingEl,TIME_SETTING_NOTIFICATION_TYPE,timeSettingVal);
		}

		return timeSettingVal;
	}
	
	private Map<String,String> getElemtntFunctionValue(Element element, String elementName,Map<String,String> storedMap){
		Element operatingFunctionTypeEl = element.element(elementName);
		if(operatingFunctionTypeEl!=null){
			storedMap.put(operatingFunctionTypeEl.getTagName(), operatingFunctionTypeEl.getText());
		}
		return storedMap;
	}
