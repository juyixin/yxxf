package com.eazytec.webapp.spring;
 ...
 * add validation configuration for their own persistent classes to the application without modifying any of the
 * existing Spring configuration or jar distribution files.

 * <p>As an example consider the following Spring bean configuration:

 * <pre>
 * &lt;bean class=&quot;com.eazytec.webapp.spring.ValidatorExtensionPostProcessor&quot;&gt;
 ...
 * process coupled with the configuration file auto detection will ensure that the validation extension is
 * automatically included into the application without requiring any modification of existing config files.
 *
 * @author madan
 */
