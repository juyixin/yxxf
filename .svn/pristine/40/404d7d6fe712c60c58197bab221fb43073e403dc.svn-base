...
//	  }
  ...
  private boolean isBelongsToSameTask(VariableInstanceEntity existingVariableInstance, ExecutionEntity currentActivityExecution){
	  if(StringUtil.isEmptyString(existingVariableInstance.getTaskId())){
		  if(StringUtil.isEmptyString(currentActivityExecution.getPresentTaskId())){
			  return true;
		  }else{
			  return false;
		  }
	  }else{
		  return existingVariableInstance.getTaskId().equals(currentActivityExecution.getPresentTaskId());
	  }
  }
...
//		    }
		    // TODO Check if the above commented out is valid or a temp fix
		    if ((variableInstance != null) && !isBelongsToSameTask(variableInstance, sourceActivityExecution)) {  
		    	removeVariable(variableName);
		    	variableInstance = null;             
		    }
		    if (variableInstance == null) {
  ...
  protected void createVariableLocal(String variableName, Object value, ExecutionEntity sourceActivityExecution) {
    ...
    if (variableInstances.containsKey(variableName) && isBelongsToSameTask(variableInstances.get(variableName), sourceActivityExecution)) {
  ...
  protected void deleteVariableInstanceForExplicitUserCall(VariableInstanceEntity variableInstance, ExecutionEntity sourceActivityExecution) {
    variableInstance.delete();
    //TODO set value null temp removed
