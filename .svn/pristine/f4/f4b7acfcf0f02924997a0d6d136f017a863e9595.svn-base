...
CKEDITOR.plugins.add( 'userlist', {
	init: function( editor ){
		editor.addCommand( 'showUserListInDialog', new CKEDITOR.dialogCommand( 'userlist' ) );
		CKEDITOR.dialog.add( 'userlist', function( editor ) {
			var autoAttributes =
			{
				value : 1,
				size : 1,
			...
			return {
				...
				onOk : function()
				{
					...
					if ( isInsertMode ){
						...
						var fakeElement = editor.createFakeElement( element, 'cke_userlist', 'text', true );
						...
						elements : [
							{
								type : 'html',
								align : 'left',
								html : '<span class="color-red">*</span><span class="form-title-msg">'+form.msg.userlist+'</span>'
							}, 
							{
								id : '_cke_saved_name',
								type : 'text',
								...
								{
									var userListName = 	element.data( 'cke-saved-name' ) || element.getAttribute( 'name' ) || '' ;
									var user = "";
									if(userListName != '' && userListName.indexOf("_") > -1){
										user = userListName.split("_");
									}
									
									this.setValue(user[0]);
							...
							},
							{
								...
								[
									{
									    ...
									    onChange : function() {
									         ...
									         getAllColumnByTableId(element_id, this.getValue(), 'userlist');
									    },
									    commit : function( element )
										{
									    	//var element = data.element;
									...
									},
									{
										...
										onLoad : function(){
									    	var element_id = '#' + this.getInputElement().$.id;
									    	getAllColumnByTableId(element_id, $("#tableId").val(), 'userlist');
									    },
									    commit : function( element )
										{
									    	//var element = data.element;
								...
								items :
								[
									[ editor.lang.forms.textfield.typeText, 'userlist' ]
								],
								setup : function( element )
								{
