...
}if(l){var p=this.facade.getStencilSets();
var a=p[p.keys()[0]].source().split("stencilsets")[1];
...
if(!q[1]){if(h.include(ORYX.CONFIG.ORYX_NEW_URL)){var p=this.facade.getStencilSets().values()[0];
var e={title:ORYX.I18N.Save.newProcess,summary:"",type:p.title(),url:h,namespace:p.namespace()};
...
var c=new Ext.Panel({labelAlign:"right",title:ORYX.I18N.Save.dialogDesciption,width:450,bodyStyle:"padding:5px 5px 0 0",items:[{xtype:"form",monitorValid:true,id:"processForm",name:"processForm",layout:"form",border:false,items:[{xtype:"textfield",fieldLabel:ORYX.I18N.Save.dialogLabelName+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"title",allowBlank:false,labelStyle:"font-weight:bold;",regex:/^[A-Za-z\u4e00-\u9eff\.][A-Za-z\' \'\w\u4e00-\u9eff\.][\-a-zA-Z\_\u4e00-\u9eff\s]{1,100}$/,anchor:"90%"},{xtype:"textarea",grow:true,fieldLabel:ORYX.I18N.Save.dialogLabelDesc+" ",name:"summary",labelStyle:"font-weight:bold;",anchor:"90%"},{xtype:"hidden",grow:true,name:"namespace",value:p.namespace()},{xtype:"textfield",fieldLabel:'排序<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"orderNo",allowBlank:false,labelStyle:"font-weight:bold;",regex:/^[1-9]{1,2}$/,anchor:"90%"},{xtype:"combo",store:d,id:"classification_type",name:"classification_type",fieldLabel:ORYX.I18N.Save.dialogLabelClassification+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',labelStyle:"font-weight:bold;",displayField:"name",valueField:"id",allowBlank:false,typeAhead:true,mode:"local",lazyRender:true,triggerAction:"all",overflow:"auto",emptyText:"请选择流程分类...",editable:false,selectOnFocus:true,anchor:"90%",regex:new RegExp("^[A-Za-z一-黿.][A-Za-z w一-黿.]{1,40}$")}],buttons:[{text:ORYX.I18N.Save.saveBtn,formBind:true,handler:function(){callback()
...
var s=x.findField("summary").getValue().strip();
s=s.length==0?e.summary:s;
var r=x.findField("classification_type").getRawValue().strip();
var v=x.findField("classification_type").getValue().strip();
var t=x.findField("namespace").getValue().strip();
t=t.length==0?e.namespace:t;
var y=x.findField("orderNo").getValue().strip();
win.destroy();
this.sendSaveRequest(h,{data:this.serializedDOM,svg:b,title:w,summary:s,type:t,classification_type:v,orderNo:y,designMode:"create"},l,this.mask)
}get .bind(this);
...
d.load();
var p=this.facade.getStencilSets().values()[0];
var e={title:ORYX.I18N.Save.newProcess,summary:"",type:p.title(),url:h,namespace:p.namespace()};
var m=decodeURIComponent(q[1].split("&")[2].split("=")[1]);
var k=decodeURIComponent(q[1].split("&")[3].split("=")[1]);
var o=decodeURIComponent(q[1].split("&")[4].split("=")[1]).replace("#","");
var n=decodeURIComponent(q[1].split("&")[5].split("=")[1]);
var c=new Ext.Panel({labelAlign:"right",title:ORYX.I18N.Save.dialogDesciption,width:450,bodyStyle:"padding:5px 5px 0 0",items:[{xtype:"form",monitorValid:true,id:"processForm",name:"processForm",layout:"form",border:false,items:[{xtype:"textfield",fieldLabel:ORYX.I18N.Save.dialogLabelName+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"title",allowBlank:false,disabled:true,value:m,labelStyle:"font-weight:bold;",regex:/^[A-Za-z\u4e00-\u9eff\.][A-Za-z\' \'\w\u4e00-\u9eff\.][\-a-zA-Z\_\u4e00-\u9eff\s]{1,100}$/,anchor:"90%"},{xtype:"textarea",grow:true,fieldLabel:ORYX.I18N.Save.dialogLabelDesc+" ",name:"summary",value:k,labelStyle:"font-weight:bold;",anchor:"90%"},{xtype:"hidden",grow:true,name:"namespace",value:p.namespace()},{xtype:"textfield",fieldLabel:'排序<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"orderNo",value:n,allowBlank:false,labelStyle:"font-weight:bold;",regex:/^[1-9]{1,2}$/,anchor:"90%"},{xtype:"combo",store:d,id:"classification_type",name:"classification_type",fieldLabel:ORYX.I18N.Save.dialogLabelClassification+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',labelStyle:"font-weight:bold;",displayField:"name",valueField:"id",allowBlank:false,typeAhead:true,mode:"local",lazyRender:true,triggerAction:"all",overflow:"auto",emptyText:"请选择流程分类...",editable:false,selectOnFocus:true,anchor:"90%",regex:new RegExp("^[A-Za-z一-黿.][A-Za-z w一-黿.]{1,40}$"),listeners:{expand:function(r){r.setValue(o)
}get }}],buttons:[{text:ORYX.I18N.Save.saveBtn,formBind:true,handler:function(){callback()
}get },{text:ORYX.I18N.Save.close,handler:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE});
win.destroy()
}get .bind(this)}]}]});
callback=function(u){this.mask=new Ext.LoadMask(Ext.getBody(),{msg:ORYX.I18N.Oryx.pleaseWait});
...
var s=w.findField("summary").getValue().strip();
s=s.length==0?e.summary:s;
var r=w.findField("classification_type").getRawValue().strip();
var v=w.findField("classification_type").getValue().strip();
var t=e.namespace;
var x=w.findField("orderNo").getValue().strip();
win.destroy();
this.sendSaveRequest(h,{data:this.serializedDOM,svg:b,title:m,summary:s,type:t,classification_type:v,orderNo:x,designMode:"edit"},l,this.mask)
}get .bind(this);
win=new Ext.Window({id:"Propertie_Window",width:"auto",height:"auto",title:l?ORYX.I18N.Save.saveAsTitle:ORYX.I18N.Save.save,modal:true,bodyStyle:"background:#FFFFFF",items:c,});
win.show()
