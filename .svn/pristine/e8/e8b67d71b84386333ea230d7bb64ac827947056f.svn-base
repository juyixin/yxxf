...
<head>
    ...
    <meta name="heading" content="<fmt:message key='案例类型管理'/>"/>
</head>
...
function getNewForm(){
	...
	document.location.href = "#bpm/admin/allxForm";
    _execute('target', '');
 }
 //编辑
function _showEditAllx2(cellValue, options, rawObject) {
	return '<a id="editgroup" href="#bpm/admin/allxForm" data-role="button" data-theme="b" onclick="_execute(\'target\',\'id='
			+encodeURI(rawObject.id)+ '\');"><b>' + rawObject.lxmc + '</b></a>';
}
//删除节点
...
function onFtxxInfoSearch(){
	...
	document.location.href="#bpm/alxxgl/allx";
	_execute('target', params);
}
</script>
<input type="hidden" id="createCode"  name="createCode"/> 
	...
	<div class="row-fluid">
			<div class="page-header">
				<img src="/images/listcode.png " style="float:left; margin:9px 6px 5px 5px" /><h2><fmt:message key="案例类型管理"/></h2>
			
			<div id="header_links" align="right">
			<a style="padding: 3px; ">
		 <input id="searchText"
			type="text" placeholder="输入类型名称" style="width: 185px"></input>
		</a>
		...
		<input type="button" onclick="onFtxxInfoSearch()" value="查询"/> 
                ...
                <a class="padding3" id="deleteUsers" href="javascript:void(0);" data-role="button" data-theme="b"  onClick="deleteAllxs()"  data-icon=""><button class="btn"><fmt:message key="button.deleteDepartment"/></button></a>
		...
		<div class="span12">
			...
			<div class="span9">
				<div class="widget">
					<div class="widget-header">
						<div class="title">
							<span class="fs1" aria-hidden="true"></span><fmt:message key="案例类型管理"/>
						</div>
					</div>
					<div class="widget-body">
