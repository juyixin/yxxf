...
<script type="text/javascript">
	...
	//新增节点
	function getNewtxlForm() {
		var params = "id=" + $("#createCode").val();
		document.location.href = "#bpm/admin/addTxlry";
		_execute('target', params);
	}
		...
		$.msgBox({
			...
			success : function(result) {
				if (result == "是") {
					...
					document.location.href = "#bpm/admin/deleteTxl";
					_execute('target', params);
				} else {
					return false;
				}
	...
	function _showEditUnitUnionMessage(cellValue, options, rawObject) {
		return '<a id="one" href="#bpm/admin/txlryeditForm" data-role="button" data-theme="b" onclick="_execute(\'target\',\'id='
	...
	/* function downloadExcel(){
		document.location.href = "#bpm/admin/downloadExcel";
	    _execute('target', '');
	} */
	
	function exportExcel(){
		var params = "id=" + $("#createCode").val();
		document.location.href = "#bpm/admin/uploadMemeberForm";
	    _execute('target', params);
	} 
	
	function importExcel(){
		datas = $("#_UNITUNION_LIST").jqGrid('getGridParam', 'data');
		$("#gridDatas_form").val(JSON.stringify(datas));
		document.getElementById("exportAppraiseResult").submit();
	}
</script>


<form id="exportAppraiseResult"
	action="bpm/admin/exportBaseResult"
	method="post" style="display: none;">
	<input type="hidden" id="listViewParamsList" name="listViewParamsList"
		value="${listViewParamsList}" /> <input type="hidden" id="type" 
		name="type" /> <input type="hidden" id="gridHeader_form"
		name="gridHeader" /> <input type="hidden" id="gridDatas_form"
		name="gridDatas" /> <input type="hidden" id="gridName_form"
		name="gridName" /> <input type="hidden" id="grid_hiddenColumn"
		name="hiddenColumn" /> <input type="button" class="btn-csv"
		value="Save Form" id="export_csv_submit" />
</form>
	...
	<div class="page-header">
		...
		<div id="header_links" align="right">
			<a class="padding3" id="createUser" href="javascript:void(0);"
				...
				data-role="button" data-theme="b" onClick="deleteTxlry()" data-icon=""><button
					class="btn">
					<fmt:message key="button.delete" />
				</button></a>
				<a class="padding3" id="de1" href="javascript:void(0);"
				data-role="button" data-theme="b" onClick="exportExcel()" data-icon=""><button
					class="btn">
					<fmt:message key="导入" />
				</button></a>
				<a class="padding3" id="de2" href="javascript:void(0);"
				data-role="button" data-theme="b" onClick="importExcel()" data-icon=""><button
					class="btn">
					<fmt:message key="导出" />
				</button></a>
				<%-- <a class="padding3" id="de3" href="javascript:void(0);"
				data-role="button" data-theme="b" onClick="downloadExcel()" data-icon=""><button
					class="btn">
					<fmt:message key="下载模板"/>
				</button></a> --%>
