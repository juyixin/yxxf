...
public interface RepositoryService {
  ...
  /**
   * Gives svg string to display the process diagram
   * @param deploymentId id of the deployment, cannot be null.
   * @param resourceName name of the resource, cannot be null.
   * @throws ActivitiException when the resource doesn't exist in the given deployment or when no deployment exists
   * for the given deploymentId.
  ...
  /**
   * Gives access to a deployment resource through a stream of bytes in a List.
   * @param deploymentIds List is id of the deploymentList, cannot be null.
   * @throws ActivitiException when the deploymentIds doesn't exist 
   * for the given deploymentIds.
   */
  ...
  /** Query Method to process List of process definitions to List of Map<String, Object>
   *  
   */
  List<Map<String, Object>> createProcessDescribeQuery(List<ProcessDefinition> processDefinitions);
  ...
  List<ProcessDefinition> getProcessDefinitionByUser(String user);
  
  /**
   * Retrives all the process definitions related to a particular process key.
   * @param key process key to find all process definitions
   * @return list of process definitions related to the process key
   */
   List<ProcessDefinition> getProcessDefinitionByKey(String key);
  
   /**
    * Retrives all the process definitions related to a processDefinitionKeys.
    * @param processDefinitionIds to find all process definitions
    * @return list of process definitions related to the processDefinitionKeys
    */
   List<ProcessDefinition> getProcessDefinitionByKeys(List<String>ids);
  
  /**
   * Deletes a process definition entity, along with all the identity links, corresponding jobs etc
   * 
   * @param processDefinitionId for the process definition
   * @param deleteReason reason for deleting the definition plus if we choose to delete instances too
   * @param cascade should related instances also be deleted
   * @throws BpmException
   */
  void deleteProcessDefinitionByDefinitionId(String processDefinitionId, String deleteReason, boolean cascade)throws BpmException;
  
  /**
   * Deletes all the process definitions, along with related identities, jobs etc
   * @param processDefinitions list of definitions
   * @param deleteReason deleteReason reason for deleting the definition plus if we choose to delete instances too
   * @param cascade should related instances also be deleted
   * @throws BpmException
   */
  void deleteProcessDefinitions(List<ProcessDefinition>processDefinitions, String deleteReason, boolean cascade)throws BpmException;
  
  /**
   * Removes the processs definition entity entry from cache
   * 
   * @param processDefinitionId for the definition
   * @throws BpmException
   */
  void deleteProcessDefinitionFromCache(String processDefinitionId)throws BpmException;
  
  /**
   * Removes all the processs definitions entities entry from cache
   * 
   * @param processDefinitions list of entities
   * @throws BpmException
   */
  void deleteProcessDefinitionsFromCache(List<ProcessDefinition>processDefinitions)throws BpmException;
  /**
   * Suspends the process definition with the given List of Ids. 
   * 
   * If a process definition is in state suspended, it will not be possible to start new process instances
   * based on the process definition.
   * 
   ...
   *  @throws ActivitiException if no such processDefinition can be found or if the process definition is already in state suspended.
   */
  void suspendProcessDefinitionByIds(List<String> processDefinitionIds);
  /**
   * Suspends the process definition with the given List of Ids. 
   * 
   * If a process definition is in state suspended, it will not be possible to start new process instances
   ...
   * @throws ActivitiException if no such processDefinition can be found or if the process definition is already in state suspended.
   */
  void suspendProcessDefinitionByIds( boolean suspendProcessInstances, Date suspensionDate,List<String> processDefinitionIds);
  
 
  /**
   * Activates the process definition with the given list of ids. 
   * 
   * @throws ActivitiException if no such processDefinition can be found or if the process definition is already in state active.
   */
  void activateProcessDefinitionByIds(List<String> processDefinitionIds);
   ...
   * @throws ActivitiException if no such processDefinition can be found or if the process definition is already in state active.
   */
  void activateProcessDefinitionByIds(boolean activateProcessInstances, Date activationDate,List<String> processDefinitionIds);
  ...
  */
  ...
  /**
   * Suspend the active process instance (ie state of process instance will be suspend)
   * @param processInstnaceId
   * @throws ActivitiException if no such ProcessInstance can be found or if the process definition is already in state suspend.
   */
  void suspendProcessInstanceById(String processInstnaceId);
  
  /**
   * Active the suspend process instance (ie state of process instance will be active)
   * @param processInstnaceId
   * @throws ActivitiException if no such ProcessInstance can be found or if the process definition is already in state active.
   */
  void activateProcessInstanceById(String processInstanceId);
  
  /**
   * Gets the resource as entire entity
   * @param deploymentId
   * @param resourceName
   * @return resource
   */
  ...
  /**
   * To clear all the process Definitions from the cache. if any changes changes has been made in the table.
   */
  
  void clearProcessDefinitionsFromCache();
  
  /**
   * To get all custom operational function 
   * @return
   */
  List<CustomOperatingFunction> getAllCustomOperatingFunction();
  
  /**
   * To save custom operational function 
   */
  CustomOperatingFunction saveCustomOperatingFunction(CustomOperatingFunction customOperatingFunction,boolean isCustomOperatingUpdate);
  
  /**
   * To select custom operational function by id 
   */
  CustomOperatingFunction getCustomOperatingById(String customOperatingById);
  
  /**
   * To delete custom operational function.
   */
  void deleteCustomFunctionById(CustomOperatingFunction customOperatingObj);
