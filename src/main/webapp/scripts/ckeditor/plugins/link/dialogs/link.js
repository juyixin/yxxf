	...
	return {
		...
		onShow : function()
		{
			...
			var editor = this.getParentEditor(),
				...
				element = null;
//console.log(selection.root.$.innerHTML);
			...
			tabDivId = element.$.id;
			num = tabDivId.slice(-1);
			

			var currentDialog=CKEDITOR.dialog.getCurrent();
			currentDialog.setValueOf('info','protocol', element.getAttribute('protocol'));
			currentDialog.setValueOf('info','linkTargetType', element.getAttribute('targetoption'));
			currentDialog.setValueOf('info','url', element.getAttribute('url'));
		},
		onOk : function()
		{
			var attributes = {},
			...
			var currentDialog=CKEDITOR.dialog.getCurrent();
			var protocol = currentDialog.getValueOf('info','protocol');
			var target = currentDialog.getValueOf('info','linkTargetType');
			var url = currentDialog.getValueOf('info','url');
			var actualURL = protocol+""+url;
			var selectedText = editor.getSelection().getNative(); 
			if(selectedText == "") {
				selectedText = actualURL;
			}
		//	editor.insertHtml( "<a href='javascript:void(0);' protocol="+protocol+" url="+url+" targetoption="+target+" onClick='linkToNavigate(\""+actualURL+"\",\""+target+"\")'>"+selectedText+"<a>" );
			
			editor.insertHtml( '<span class="wizard-steps"> <span id="wizardTabSpan-'+num+'" class="completed-step">'+"<a class='fontSize13' id="+tabDivId+" href='javascript:void(0);' protocol="+protocol+" url="+url+" targetoption="+target+" onClick='linkToNavigate(\""+actualURL+"\",\""+target+"\")'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+selectedText+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a>"+'</span></span>' );	
/*			this.commitContent( data );
			...
			if ( !this._.selectedElement )
			{
				...
				if ( ranges.length == 1 && ranges[0].collapsed )
				{
					...
					alert(attributes[ 'href' ]);
					alert(attributes[ 'target' ]);
					console.log(attributes);
					//ranges[0].insertNode( text );
					editor.insertHtml( "<a href='javascript:void(0);' onClick='linkToNavigate(\""+attributes[ 'href' ]+"\",\""+attributes[ 'target' ]+"\")'>"+attributes[ 'href' ]+"<a>" );
