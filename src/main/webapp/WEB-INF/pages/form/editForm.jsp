...
<%-- <c:out value="${formId}"></c:out><br/>
<c:out value="${xmlString}"></c:out><br/> --%>
<head>
...
<div class="span10" id="sourceView">
	<div class="row-fluid background-white">
		<div class="span12">
			<div class="titleBG">
	     		<span class="floatLeft fontSize14 pad-L10 pad-T10"><fmt:message key="form.editForm"/> - ${form.formName}</span>
	     		...
	     		<div class="floatRight pad-T2">
	     			<ul id="tabs"><li id="tabs-1" class="selected"><fmt:message key="form.design "/></li><li id="tabs-2"><fmt:message key="form.hTMLSource"/></li><!-- <li id="tabs-3">XML Source</li> --><li id="tabs-4"><fmt:message key="form.preview"/></li></ul>
	     		</div>
			</div>
		</div>
	</div>
				   	...
				   	<div class="row-fluid background-white">
						...
						<div class="span1">
							<div id="submitForm" class="mar-B15 mar-R10 mar-T5" align="right">
								<form:form  commandName="form" method="post" action="bpm/form/saveForm" autocomplete="off" modelAttribute="form" id="saveForm" onClick="cancelFullScreenMode('target');">
									<%-- <form:hidden path="formName" />
									<form:hidden path="id" />
									<form:hidden path="xmlSource" /> --%>
										<input type="hidden" id="formName" name="formName" value="${form.formName}"/>
										<input type="hidden" id="id" name="id" value="${form.id}" />   
										<input type="hidden" id="version" name="version" value="${form.version}"/>
										<input type="hidden" id="description" name="description" value="${form.description}"/>
										<input type="hidden" id="htmlSource" name="htmlSource" />
										<input type="hidden" id="xmlSource" name="xmlSource" />
										<input type="submit" value="Update Form" class="btn btn-primary updateFormButton padding0 line-height0"/>
								</form:form>
							</div>
							</div>
						</div>
			   	...
			   	<div id="tab-content-2" class="textalign-left height-per-95">
			   		<div class="row-fluid background-white height-per-95">
				   		 ...
				   		 <div id="updateForm" class="mar-B15 mar-R10" align="right">
	                           <input type="button" value="Apply" onclick= "load_html();" class="btn btn-primary updateFormButton"/>
	                     </div>
			   		</div>
			   	</div>
...
<div id="form_button_dialog"></div>
    <!-- end new ui  -->
<div class="clear"></div>
<input type="hidden" id="xmlString" value="${form.xmlString}"/>
<script type="text/javascript">
var selected = null ;
var source = $("#xmlString").val();
$("#xmlsource").text(source);
$("#xmlSource").val(source);
load_xml();
...
$(document).ready(function(){ 
	...
	$('#addSubForm').click(function() { addFormPanel();});
	$('#addTemplate').click(function() {addTemplatePanel();});
